...
export default function MissionControlScreen({ route }: any) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.progressWrapper, { paddingHorizontal: tokens.gutter, paddingVertical: sp(14), borderBottomWidth: tokens.rule.hair, borderBottomColor: theme.hairline }]}>
      ...
      <View style={styles.mainContent}>
        <View style={[styles.topRow, { padding: tokens.gutter }]}>
          <View style={[styles.mapColumn, { paddingRight: tokens.gutter }]}>
        ...
        <View style={[styles.bottomSection, { paddingHorizontal: tokens.gutter, paddingTop: sp(16), paddingBottom: sp(14) }]}>
          <View style={styles.controlsRow}>
            <View style={[styles.gimbalWrapper, { paddingRight: tokens.gutter }]}>
              <GimbalControlPad onPanTilt={(p, y) => telemetryService.sendGimbalCommand(droneId, { pitch: p, yaw: y })} onZoom={(z) => telemetryService.sendGimbalCommand(droneId, { zoomLevel: z })} onPhoto={() => telemetryService.sendGimbalCommand(droneId, { isPhotoMode: true })} onRecordToggle={() => telemetryService.sendGimbalCommand(droneId, { isRecording: true })} />
            </View>
            <View style={{ width: tokens.rule.hair, alignSelf: 'stretch', backgroundColor: theme.hairline }} />
            <View style={[styles.telemetryWrapper, { paddingLeft: tokens.gutter }]}>
      ...
      <View style={[styles.commandStrip, { borderTopColor: theme.textPrimary, borderTopWidth: tokens.rule.medium, backgroundColor: theme.surface, paddingHorizontal: tokens.gutter, paddingVertical: sp(12) }]}>
...
const styles = StyleSheet.create({
  ...
  mainContent: { flex: 1 },
  commandStrip: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center' },
  droneStatus: { fontFamily: typography.fonts.medium, fontSize: typography.sizes.xs, letterSpacing: 0.5, marginTop: 2 },
  actionsRow: { flexDirection: 'row' },
  actionBtn: { flexDirection: 'row', alignItems: 'center', marginHorizontal: 10 },
  actionBtnText: { fontFamily: typography.fonts.bold, fontSize: typography.sizes.xs },
  topRow: { flex: 1, flexDirection: 'row' },
  videoColumn: { flex: 1 },
  mapColumn: { flex: 1.1 },
  videoFeedWrapper: { flex: 1, overflow: 'hidden' },
  mapContainerSquare: { flex: 1 },
  bottomSection: { flex: 0.6 },
  controlsRow: { flex: 1, flexDirection: 'row', alignItems: 'stretch' },
  gimbalWrapper: { flex: 1, justifyContent: 'center' },
  telemetryWrapper: { flex: 2.2 },
  progressWrapper: {},
