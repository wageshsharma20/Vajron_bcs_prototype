...
export default function MissionControlScreen({ route }: any) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <PageHeader title={droneId} subtitle={<Text style={[styles.droneStatus, { color: headerColors.muted }]}>{telemetry?.flightMode?.toUpperCase() || 'UNKNOWN'} · {telemetry?.gpsFixType?.toUpperCase() || 'NO'} FIX</Text>} />
      ...
      <View style={styles.mainContent}>
        <View style={[styles.threeColRow, { padding: tokens.gutter }]}>
          <View style={[styles.colLeft, { paddingRight: tokens.gutter, borderRightWidth: tokens.rule.hair, borderRightColor: theme.hairline }]}>
            <View style={{ flex: 1, paddingBottom: tokens.gutter }}><TelemetryHUD telemetry={telemetry} isGrid={false} /></View>
            <Rule weight="hair" color={theme.hairline} />
            <View style={{ flex: 1, paddingTop: tokens.gutter }}><GimbalControlPad onPanTilt={(p, y) => telemetryService.sendGimbalCommand(droneId, { pitch: p, yaw: y })} onZoom={(z) => telemetryService.sendGimbalCommand(droneId, { zoomLevel: z })} onPhoto={() => telemetryService.sendGimbalCommand(droneId, { isPhotoMode: true })} onRecordToggle={() => telemetryService.sendGimbalCommand(droneId, { isRecording: true })} /></View>
          </View>
          <View style={[styles.colMiddle, { paddingHorizontal: tokens.gutter }]}>
            <VideoFeedPlayer telemetry={telemetry} player={feedPlayer} isArmed={isArmed} />
          </View>
          <View style={[styles.colRight, { paddingLeft: tokens.gutter }]}>
        ...
        <View style={[styles.bottomSection, { paddingHorizontal: tokens.gutter, paddingVertical: sp(12), backgroundColor: theme.surface }]}>
          <View style={{ flex: 1 }}><MissionProgressBar totalWaypoints={TOTAL_WAYPOINTS} currentWaypoint={currentWaypoint} /></View>
          <View style={{ marginLeft: tokens.gutter }}>
...
const styles = StyleSheet.create({
  ...
  threeColRow: { flex: 1, flexDirection: 'row' },
  colLeft: { flex: 0.8 },
  colMiddle: { flex: 1.5, overflow: 'hidden' },
  colRight: { flex: 1.5, overflow: 'hidden' },
  bottomSection: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
