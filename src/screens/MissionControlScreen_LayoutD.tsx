...
export default function MissionControlScreen({ route }: any) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.topStrip, { borderBottomWidth: tokens.rule.thick, borderBottomColor: theme.textPrimary, backgroundColor: theme.surface }]}>
        <View style={{ width: 250 }}><PageHeader title={droneId} subtitle={<Text style={[styles.droneStatus, { color: headerColors.muted }]}>{telemetry?.flightMode?.toUpperCase()}</Text>} /></View>
        <View style={{ flex: 1, paddingHorizontal: tokens.gutter }}><MissionProgressBar totalWaypoints={TOTAL_WAYPOINTS} currentWaypoint={currentWaypoint} /></View>
        <View style={{ paddingRight: tokens.gutter }}>
      ...
      <View style={styles.grid}>
        <View style={styles.gridRow}>
          <View style={[styles.gridCell, { borderRightWidth: tokens.rule.hair, borderRightColor: theme.hairline, borderBottomWidth: tokens.rule.hair, borderBottomColor: theme.hairline, padding: tokens.gutter }]}><VideoFeedPlayer telemetry={telemetry} player={feedPlayer} isArmed={isArmed} /></View>
          <View style={[styles.gridCell, { borderBottomWidth: tokens.rule.hair, borderBottomColor: theme.hairline, padding: tokens.gutter }]}><MissionMapView player={mapPlayer} isArmed={isArmed} progress={missionProgress} /></View>
        </View>
        <View style={styles.gridRow}>
          <View style={[styles.gridCell, { borderRightWidth: tokens.rule.hair, borderRightColor: theme.hairline, padding: tokens.gutter }]}><GimbalControlPad onPanTilt={(p, y) => telemetryService.sendGimbalCommand(droneId, { pitch: p, yaw: y })} onZoom={(z) => telemetryService.sendGimbalCommand(droneId, { zoomLevel: z })} onPhoto={() => telemetryService.sendGimbalCommand(droneId, { isPhotoMode: true })} onRecordToggle={() => telemetryService.sendGimbalCommand(droneId, { isRecording: true })} /></View>
          <View style={[styles.gridCell, { padding: tokens.gutter }]}><TelemetryHUD telemetry={telemetry} isGrid={true} /></View>
        </View>
      </View>
    </View>
  );
